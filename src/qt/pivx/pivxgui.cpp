...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Mobolith Core client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Mobolith Core"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Mobolith Core"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
